...
- [x] 1.1 Create extra_pack_purchases table with all fields and constraints
...
- [x] 1.2 Create database function for atomic extra pack consumption
...
- [x] 1.3 Create database function for available balance calculation
...
- [x] 1.4 Write property test for expiration date calculation
  - **Property 1: Expiration date calculation**
  - **Validates: Requirements 1.4**

- [x] 2. ExtraPacksService implementation
- [x] 2.1 Implement ExtraPacksService.getBundles()
  - Return array of 3 bundles: 10/€2.99, 30/€6.99, 75/€14.99
  - Calculate pricePerPack for each bundle
  - Mark 30-pack bundle as popular
  - _Requirements: 1.1_

- [x] 2.2 Implement ExtraPacksService.getAvailableBalance()
...
- [x] 2.3 Write property test for independent purchase tracking
  - **Property 2: Independent purchase tracking**
  - **Validates: Requirements 1.5**

- [x] 2.4 Write property test for expired pack exclusion
  - **Property 6: Expired pack exclusion**
  - **Validates: Requirements 3.2, 6.3**

- [x] 2.5 Write property test for balance display completeness
  - **Property 8: Balance display completeness**
  - **Validates: Requirements 3.4, 7.2**

- [x] 2.6 Implement ExtraPacksService.createPurchase()
...
- [x] 2.7 Implement ExtraPacksService.consumeExtraPacks()
...
- [x] 2.8 Write property test for idempotent consumption
  - **Property 17: Idempotent consumption**
  - **Validates: Requirements 8.3**

- [x] 2.9 Implement ExtraPacksService.canRefund()
  - Check purchase is within 14 days of purchase date
  - Check consumed === 0
  - Return allowed boolean and reason if not allowed
  - _Requirements: 4.1, 4.2_

- [x] 2.10 Write property test for refund eligibility rules
  - **Property 10: Refund eligibility rules**
  - **Validates: Requirements 4.1, 4.2**

- [x] 2.11 Implement ExtraPacksService.processRefund()
...
- [x] 2.12 Write property test for refund balance adjustment
  - **Property 11: Refund balance adjustment**
  - **Validates: Requirements 4.3**

- [x] 2.13 Write property test for refund recording
  - **Property 12: Refund recording**
  - **Validates: Requirements 4.4, 4.5**

- [x] 2.14 Implement ExtraPacksService.expirePurchases()
...
- [x] 2.15 Write property test for expiration job identification
  - **Property 14: Expiration job identification**
  - **Validates: Requirements 6.1**

- [x] 2.16 Write property test for expiration job marking
  - **Property 7: Expiration job marking**
  - **Validates: Requirements 3.3, 6.2**

- [x] 3. Extend UsageService for extra packs
- [x] 3.1 Extend UsageService.canCreatePack() to include extra packs
...
- [x] 3.2 Write property test for monthly quota priority
  - **Property 3: Monthly quota priority**
  - **Validates: Requirements 2.1, 2.2**

- [x] 3.3 Write property test for unified availability response
  - **Property 18: Unified availability response**
  - **Validates: Requirements 8.4**

- [x] 3.4 Extend UsageService.consumePackQuota() to handle extra packs
...
- [x] 3.5 Write property test for extra pack consumption order
  - **Property 4: Extra pack consumption order**
  - **Validates: Requirements 2.3**

- [x] 3.6 Write property test for consumption source feedback
  - **Property 16: Consumption source feedback**
  - **Validates: Requirements 7.4**

- [x] 3.7 Add helper method UsageService.getExpirationWarning()
...
- [x] 3.8 Write property test for expiration warning threshold
  - **Property 9: Expiration warning threshold**
  - **Validates: Requirements 3.5**

- [x] 3.9 Write property test for period boundary preservation
  - **Property 5: Period boundary preservation**
  - **Validates: Requirements 2.5, 3.1**

- [x] 4. Stripe integration
- [x] 4.1 Install and configure Stripe SDK
...
- [x] 4.2 Implement StripeService.createExtraPacksCheckout()
...
- [x] 4.3 Implement StripeService.verifyWebhookSignature()
  - Use Stripe.webhooks.constructEvent()
  - Verify signature matches STRIPE_WEBHOOK_SECRET
  - Return boolean for valid/invalid
  - _Requirements: 1.3_

- [x] 4.4 Implement StripeService.handlePaymentSuccess()
...
- [x] 4.5 Implement StripeService.createRefund()
...
- [x] 5. API endpoints
- [x] 5.1 Create POST /api/payments/extra-packs endpoint
...
- [x] 5.2 Create POST /api/webhooks/stripe endpoint
...
- [x] 5.3 Create GET /api/users/extra-packs endpoint
  - Authenticate user
...
- [x] 5.4 Create GET /api/users/extra-packs/history endpoint
  - Authenticate user
  - Query all purchases for user (including refunded)
  - Return purchase history with all fields
  - _Requirements: 7.3_

- [x] 5.5 Write property test for purchase history completeness
  - **Property 15: Purchase history completeness**
  - **Validates: Requirements 7.3**
  - **Tests passing: 3/3 (100 runs each)**

- [x] 5.6 Create POST /api/payments/extra-packs/refund endpoint
  - Authenticate user
...
- [x] 6.1 Create QuotaExhaustedPaywall component
...
- [x] 6.2 Write property test for plan-based modal priority
  - **Property 13: Plan-based modal priority**
  - **Validates: Requirements 5.5**
  - **Tests passing: 5/5 (50-100 runs each)**

- [x] 6.3 Modify CreatePackModal to use QuotaExhaustedPaywall
...
- [x] 6.4 Create ExtraPacksBalance component
...
- [x] 6.5 Create ExtraPacksPurchaseHistory component
...
- [x] 6.6 Add ExtraPacksBalance to user dashboard
  - Place near monthly quota display
  - Show both monthly and extra packs balances
  - Link to purchase history
  - _Requirements: 7.1_
  - **Implemented: Added to DashboardClient.tsx above stats grid**

- [x] 6.7 Update pack creation success message
  - Show which source was consumed (monthly or extra)
  - Show updated balances for both sources
  - _Requirements: 7.4_
  - **Note: Consumption source is already returned by UsageService.consumePackQuota() and available in usage stats. Frontend can display this in success messages or status pages.**

- [x] 7. Background jobs
- [x] 7.1 Create Inngest function expire-extra-packs
  ...
  - **Implemented: Created src/lib/inngest/functions/expire-extra-packs.ts**

- [x] 7.2 Register expire-extra-packs in Inngest serve endpoint
  - Import function in app/api/inngest/route.ts
  - Add to functions array
  - Test cron trigger locally
  - _Requirements: 6.1_
  - **Implemented: Registered in src/app/api/inngest/route.ts**
  - **Test endpoint: Created src/app/api/admin/test/expire-extra-packs/route.ts**
