 ...
 * Export flashcards to Anki format (Student Pro+ only)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Check plan (Student Pro or Pro+ required)
    ...
    const ankiBuffer = await ExportServiceServer.generateFlashcardsAnki(flashcards)

    // Return Anki package as response
    return new NextResponse(Buffer.from(ankiBuffer), {
      headers: {
        'Content-Type': 'application/octet-stream',
        'Content-Disposition': `attachment; filename="${pack.title.replace(/[^a-z0-9]/gi, '_')}_flashcards.apkg"`,
      },
    })
  } catch (error) {
    console.error('Flashcards Anki export error:', error)
    return NextResponse.json(
      { error: 'Failed to export flashcards to Anki' },
      { status: 500 }
    )
  }
}
