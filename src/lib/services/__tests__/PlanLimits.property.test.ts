    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (planTier) => {
          ...
          if (planTier === 'free') {
            expect(limits.packsPerMonth).toBe(5)
            expect(limits.cardsPerPack).toBe(40)
            expect(limits.questionsPerQuiz).toBe(15)
            expect(limits.mindmapNodesLimit).toBe(80)
            expect(limits.priorityProcessing).toBe(false)
          } else if (planTier === 'student_pro') {
            expect(limits.packsPerMonth).toBe(60)
            ...
            expect(limits.packsPerMonth).toBe(300)
            expect(limits.cardsPerPack).toBe(300)
